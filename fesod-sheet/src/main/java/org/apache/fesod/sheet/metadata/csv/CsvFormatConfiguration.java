...
public class CsvFormatConfiguration {
    ...
    private CsvParserSettings createParserSettings() {
        ...
        // Enable parallel I/O for improved throughput
        settings.setReadInputOnSeparateThread(true);

        // Disable column reordering — CSV columns are always positional
        settings.setColumnReorderingEnabled(false);

        // Use 2 MB input buffer for improved I/O throughput on large files
    ...
    public CsvWriterSettings toWriterSettings() {
        if (cachedWriterSettings == null) {
            synchronized (this) {
                if (cachedWriterSettings == null) {
                    cachedWriterSettings = createWriterSettings();
                }
            }
        }
        return cachedWriterSettings;
    }

    private CsvWriterSettings createWriterSettings() {
